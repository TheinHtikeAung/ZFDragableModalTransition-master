...
- (void)setDragable:(BOOL)dragable
{
    ...
    if (_dragable) {
        ...
        self.gesture.direction = _direction;
...
//    }
    ...
    if (_direction != ZFModalTransitonDirectionBottom && _direction != ZFModalTransitonDirectionTop) {
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    if (!self.isDismiss) {
        ...
        if (self.direction == ZFModalTransitonDirectionTop) {
        ...
        }else if (self.direction == ZFModalTransitonDirectionBottom) {
            startRect = CGRectMake(0,
                                   CGRectGetHeight(containerView.frame),
                                   CGRectGetWidth(containerView.bounds),
                                   CGRectGetHeight(containerView.bounds));
...
- (void)handlePan:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (velocityForSelectedDirection > 100
            ...
            && (self.direction == ZFModalTransitonDirectionLeft
                || self.direction == ZFModalTransitonDirectionTop)) {
                [self finishInteractiveTransition];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (_direction == ZFModalTransitonDirectionTop) {
        
        CGFloat contentHeight = self.scrollview.contentSize.height;
        CGFloat topVerticalOffset = _scrollview.contentOffset.y + _scrollview.frame.size.height;
        
        NSLog(@"Y Content OffSet %f", topVerticalOffset);
        NSLog(@"Content Size %f", contentHeight);
        
        if ((fabs(velocity.x) < fabs(velocity.y)) && (nowPoint.y < prevPoint.y) && (topVerticalOffset >= contentHeight)) {
            self.isFail = @NO;
        } else if (topVerticalOffset <= contentHeight) {
            self.state = UIGestureRecognizerStateFailed;
            self.isFail = @YES;
        } else {
            self.isFail = @NO;
        ...
        NSLog(@"Y Content OffSet %f", self.scrollview.contentOffset.y);
        NSLog(@"Y Content OffSet %f", topVerticalOffset);
